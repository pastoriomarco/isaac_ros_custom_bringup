...
log() {
  echo "[isaac-ros-manipulation-bootstrap] $*"
}

ISAAC_ROS_WS="${ISAAC_ROS_WS:-/workspaces/isaac_ros-dev}"
FORCE="${ISAAC_ROS_4_4_BOOTSTRAP_FORCE:-0}"
...
USE_CYCLONEDDS="${ISAAC_ROS_4_4_USE_CYCLONEDDS:-0}"

if [[ "${ISAAC_ROS_4_4_BOOTSTRAP:-1}" != "1" ]]; then
  log "Skipping Isaac ROS manipulation bootstrap because ISAAC_ROS_4_4_BOOTSTRAP=${ISAAC_ROS_4_4_BOOTSTRAP}."
  exit 0
fi
...
if [[ "${USE_CYCLONEDDS}" == "1" ]]; then
  ...
  log "Using CycloneDDS RMW because ISAAC_ROS_4_4_USE_CYCLONEDDS=1."
elif [[ "${RMW_IMPLEMENTATION:-}" == "rmw_cyclonedds_cpp" ]]; then
  log "CycloneDDS RMW is active from the existing environment."
else
  if [[ -z "${RMW_IMPLEMENTATION:-}" ]]; then
    export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
    log "RMW_IMPLEMENTATION was unset; defaulting to rmw_fastrtps_cpp for Isaac Sim compatibility."
  fi
  log "WARNING: RMW_IMPLEMENTATION is '${RMW_IMPLEMENTATION}', not 'rmw_cyclonedds_cpp'."
  log "Isaac for Manipulation docs recommend CycloneDDS for ROS workflows; Isaac Sim commonly uses Fast DDS."
  log "Set ISAAC_ROS_4_4_USE_CYCLONEDDS=1 to opt into CycloneDDS for this startup hook."
fi
...
if apt_pkg_installed ros-jazzy-isaac-ros-manipulation-bringup; then
  log "ros-jazzy-isaac-ros-manipulation-bringup already installed; skipping"
else
  log "Refreshing apt package lists"
  sudo apt-get update
  log "Installing ros-jazzy-isaac-ros-manipulation-bringup"
