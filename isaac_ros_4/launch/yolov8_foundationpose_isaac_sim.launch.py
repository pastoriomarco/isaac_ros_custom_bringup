...
# Isaac ROS 4.4 (ROS 2 Jazzy) port of yolov8_foundationpose_realsense_remote.launch.py.
#
# Pipeline: a (simulated) RealSense stream published by Isaac Sim ->
#   NitrosCameraDrop -> DNN image encoder -> TensorRT (custom YOLOv8) -> YoloV8 decoder
#   -> Detection2DArrayFilter -> Detection2DToMask -> unletterbox crop -> resize
#   -> FoundationPose (RGB + depth + camera_info + mask), with optional tracking.
#
# Depth is taken DIRECTLY from the sim RealSense aligned-depth topic (no ESS), so this
# differs from the stock isaac_ros_foundationpose_isaac_sim.launch.py (RT-DETR + ESS).
#
# 4.4 deltas vs the 3.x source (verified against release-4.4):
#   * CropNode no longer accepts `encoding_desired` (removed here).
#   * dnn_image_encoder.launch.py is retained; TensorRT/decoder use default topics
#     (encoder tensor_output_topic=/tensor_pub -> TensorRT `tensor_pub` -> `tensor_sub`
#      -> decoder -> detections_output), so no tensor remaps are needed.
#   * Defaults target the Isaac Sim sim-RealSense scene (/image_rect, /camera_info, /depth float32).
#
# NOTE: REALSENSE_IMAGE_WIDTH/HEIGHT must match the camera stream your Isaac Sim scene
# publishes (RealSense default 1280x720). If your scene differs, edit these constants
# (the letterbox/crop math derives from them).
...
def generate_launch_description():
    ...
    launch_args = [
        ...
        # NitrosCameraDrop throttle (X kept / every Y); matches upstream RealSense behavior
        ...
        # Set True once to (re)build the YOLOv8 .plan from the .onnx (e.g. after a TensorRT
        # version change). Leave False for normal runs to skip the warm-up rebuild.
        ...
        # Depth handling: True if depth is 32FC1 meters (Isaac Sim). Set False for 16UC1 mm.
    ...
    # Drop nodes: throttle + pick depth NITROS format
    ...
    # DNN image encoder (letterbox REALSENSE -> 640x640, planar tensor on /tensor_pub).
    # 4.4 still ships dnn_image_encoder.launch.py; it attaches to our shared container.
    # mean=0 / stddev=1 keep YOLOv8's expected 0..1 scaling (NOT the encoder's 0.5 default).
    ...
    # TensorRT (custom YOLOv8 engine). Default topics: in `tensor_pub` (=/tensor_pub),
    # out `tensor_sub` -> consumed by the decoder. No remaps needed (matches 4.4 upstream).
    ...
    # Binary mask at YOLO network resolution (640x640 letterboxed)
    ...
    # Remove top/bottom letterbox padding: 640x640 -> 640x360 (content only).
    # 4.4 CropNode has NO encoding_desired param (dropped vs 3.x); mono8 is preserved.
    ...
    # Upscale mask 640x360 -> full RealSense res so RGB/depth/mask match for FoundationPose
