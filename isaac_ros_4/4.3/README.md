# Isaac Manipulation layer (Isaac ROS 4.3) — `isaac-ros` CLI integration
...
The `4.3` bootstrap hooks rerun the required setup in-container and skip `git clone` when the target
...
The 4.3 `isaac_manipulation` image defaults to `rmw_fastrtps_cpp` for Isaac Sim-oriented flows.
If you need CycloneDDS for a real robot workflow, override it before launching:
...
`CONFIG_DOCKER_SEARCH_DIRS`. Do the same for `4.3/`.
...
CONFIG_DOCKER_SEARCH_DIRS=(/etc/isaac-ros-cli/docker ${ISAAC_ROS_WS}/docker ${ISAAC_ROS_WS}/src/isaac_ros_custom_bringup/isaac_ros_4/4.3)
EOF
```

Notes:
- If you **also** have the `4.2/`, `4.1/`, or `4.0/` directory in `CONFIG_DOCKER_SEARCH_DIRS`, put `4.3/` **before** them
