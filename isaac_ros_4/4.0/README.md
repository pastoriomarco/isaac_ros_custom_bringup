...
2. Create the Isaac ROS CLI helper files (recommended: run the bootstrap script):
   ...
   **WARNING**: update the helper files in this section according to your needs!
   ...
   ```bash
   bash ${ISAAC_ROS_WS}/src/isaac_ros_custom_bringup/isaac_ros_4/4.0/scripts/bootstrap_isaac_ros_cli_files.sh
   ```

   To use the source build layer, rerun with `--source`:

   ```bash
   bash ${ISAAC_ROS_WS}/src/isaac_ros_custom_bringup/isaac_ros_4/4.0/scripts/bootstrap_isaac_ros_cli_files.sh --source
   ```
...
3. Prefetch NGC quickstart assets **before** starting a dev container (optional):
   ...
   ```bash
   bash ${ISAAC_ROS_WS}/src/isaac_ros_custom_bringup/isaac_ros_4/4.0/scripts/prefetch_quickstart_assets_host.sh
   ```
...
2. **Edit component config** in
   `src/isaac_ros_custom_bringup/isaac_ros_4/4.0/config/isaac_manipulation_assets.yaml`.
   - This file gates **both** model/asset installers and source build targets.
3. **Keep repos aligned** (source builds only):
   - `src/isaac_ros_custom_bringup/isaac_ros_4/4.0/source/isaac_ros_manipulation.repos`
...
```bash
bash ${ISAAC_ROS_WS}/src/isaac_ros_custom_bringup/isaac_ros_4/4.0/scripts/apply-isaac-manipulation-fixes.sh
```
...
- Configure components and versioning in `src/isaac_ros_custom_bringup/isaac_ros_4/4.0/config/isaac_manipulation_assets.yaml`.
...
and the repo list in `src/isaac_ros_custom_bringup/isaac_ros_4/4.0/source/isaac_ros_manipulation.repos`:
...
```bash
bash ${ISAAC_ROS_WS}/src/isaac_ros_custom_bringup/isaac_ros_4/4.0/scripts/print_docker_build_args_from_config.sh
```

Note: `Dockerfile.isaac_manipulation_source` always builds the repositories listed in
   `src/isaac_ros_custom_bringup/isaac_ros_4/4.0/source/isaac_ros_manipulation.repos`. The **auto-build target list**
