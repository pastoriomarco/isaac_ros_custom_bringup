...
JETSON_USER="tndlux"
DEFAULT_JETSON_HOST="192.168.1.42"
...
cleanup() {
  ...
  ssh -S "$SOCK" "${JETSON_USER}@${JETSON_HOST}" \
  ...
  ssh -S "$SOCK" -O exit "${JETSON_USER}@${JETSON_HOST}" >/dev/null 2>&1 || true
...
ssh -S "$SOCK" "${JETSON_USER}@${JETSON_HOST}" "bash -lc '
  ...
  sudo -u ${JETSON_USER} env \
    ...
    nohup x11vnc -display :0 -localhost -forever -noxdamage -nopw -rfbport ${REMOTE_PORT} \
      >/tmp/x11vnc.log 2>&1 &

  echo \$! > ${REMOTE_PID_FILE}
  sleep 0.3
  echo \"[remote] x11vnc pid=\$(cat ${REMOTE_PID_FILE})\"
'"
